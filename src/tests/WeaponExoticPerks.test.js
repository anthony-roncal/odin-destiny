...
import WeaponExoticPerks from "../components/WeaponExoticPerks";
...
describe("Weapon Exotic Perks component", () => {
    it("renders correct elements and data", () => {
        const tree = renderer.create(
            <WeaponExoticPerks
                intrinsic={mockData[0].singleInitialItemData.displayProperties}
                intrinsicIcon={`${BUNGIE_URL}${mockData[0].singleInitialItemData.displayProperties.icon}`}
                perk={mockData[3].singleInitialItemData.displayProperties}
                perkIcon={`${BUNGIE_URL}${mockData[3].singleInitialItemData.displayProperties.icon}`}
            />
        ).toJSON();
